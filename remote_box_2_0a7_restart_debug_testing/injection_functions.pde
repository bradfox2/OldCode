// this is a space to hold the functions which will be called to inject various information into the webcode
...
#define number_of_file_types 5
  ...
  char file_types[number_of_file_types][4] = { 
    "htm" , "xml" , "txt" , "csv"};
  int n;
  ...
  if (inject_flag && key =='#'){      //(file_type == "htm"||"xml"||"txt"||"csv") && key =='#'){
    Serial.print("The file type is: "); 
    Serial.println(file_type);
    Serial.print("The key is: "); 
    Serial.println(key);
    key = the_file.read(); // read the next char in the file.
    if ( key >= 'a' && key <='z') {
      Serial.print("The key is now: "); 
      Serial.println(key);
      if (_fcts[ key - 'a'] == NULL) {
        ...
        Serial.print("Calling _fcts with: "); 
        Serial.println(key);
        _fcts[ key -'a'](injection_buffer, client); 
      }  
    }
    if (injection_buffer != NULL){
      ...
      while(injection_buffer[x] != NULL){
        ...
        Serial.print(injection_buffer[x]);
        Serial.println(" from injection buffer");
...
void INJECT_current_output_mode(char *injection_buffer, Client client){
  Serial.println("INJECT_current_output_mode function was called");
  strcpy(injection_buffer, current_output_mode);
}
//void INJECT_current_ip_address(char *injection_buffer, Client client){
//  Serial.println("INJECT_current_ip_address function was called");
//  client.println((int)*current_ip_address);
//  client.println((int)*(current_ip_address + 1));
//  client.println((int)*(current_ip_address + 2));
//  client.println((int)*(current_ip_address + 3));
//}
void INJECT_current_input_definitions(char *injection_buffer, Client client){
	//page_printer(client, "insts.txt");
}
//
void INJECT_current_output_definitions(char *injection_buffer, Client client){
	//page_printer(client,"outsts.txt");
}

void INJECT_mode_change_buttons(char* injection_buffer, Client client){
        Serial.print("The number of mode buttons: "); Serial.println(number_of_mode_buttons);
        int n = 0;
        
        client.print("<form action=""/cgi-bin"" method=""post"">");  //loop to print the html for the mode change buttonss
        for (int x = 1; x <= number_of_mode_buttons; x++){
          client.print("<button type=""submit"" name=""change_the_output_mode"" value=""");
          client.print(mode_value_list[n]); // list of values [bit number] that is generated in list modes
          client.print(""">");
          client.print(mode_name_list[n]); //list of names of modes that is generated in list modes [n]::[n] dict style with mode value list
          client.print("</button>");
          n++;
        }
}
