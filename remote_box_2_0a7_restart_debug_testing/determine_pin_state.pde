...
//function to determine the pin state by comparing the input pin read against a file on the sd card.  If no pin state exists then return null

char * determine_pin_state(int input_pin_read, char file[]){  //input some 10 digit number of 1s and 0s 
    
    //Serial.print("DPS:  The file[] is: "); Serial.println(file);
    int number_of_state_pairs = 0;
    
    shutdown_ethernet();  //kick ethernet sheild off spi bus
    enable_sd(); //bring sd card onto spi bus
    
    File pin_state_file = SD.open(file);
    if (!pin_state_file) {Serial.print("DPS: Cant open the file : "); Serial.println(file);}
...
char* lookup_mode_name(char *pinset, char file[]){  //input some 10 digit number of 1s and 0s 
    ...
    if(pin_state_file){
      ...
      //if there is a match in the stored pin states, return the corresponding member of first_token_array
      for( int var = 1; var <= 100; var = var + 2){
        if(pinset == (first_token_array[var])){
         pin_state_file.close();
         enable_ethernet(); // bring ethernet back onto spi bus
         return first_token_array[var-1];
        }
