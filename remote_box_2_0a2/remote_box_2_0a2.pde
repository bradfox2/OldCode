...
void loop() {
  input_pin_read = collect_pin_states();//collect all the pin states // collect the pin states into a binary int
  //Serial.println(input_pin_read);
  //Serial.println(determine_pin_state(input_pin_read));  // compare the input pin read against a file list on the SD card and return char array pin state. copies a string to input_pin_state thats that is the current pin state
  //Serial.print("Input pin state is: "); Serial.println(input_pin_state);
